...
func Contact(w http.ResponseWriter, r *http.Request) {
	...
	if message.Name == "" || !strings.Contains(message.Email, "@") || len(message.Message) < 10 {
		writeJSON(w, http.StatusUnprocessableEntity, map[string]string{"error": "Preencha os campos corretamente"})
		return
	}
